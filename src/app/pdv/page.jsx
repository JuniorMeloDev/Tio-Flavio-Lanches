...
import { ShoppingCart, ArrowLeft, XCircle, CheckCircle, X, Printer, Ban, Plus, Copy, User } from 'lucide-react'; // Adicionado "User" aqui
...
const generatePixPayload = (key, merchantName, city, amount, txid = '***') => {
    ...
    return payloadWithCrcMarker + finalCrc;
        ...
        // --- CORREÇÃO 1: Componente do Carrinho movido para fora do componente principal ---
        const CartComponent = ({
...
export default function PdvPage() {
  ...
  // --- CORREÇÃO 2: Estados para cálculo de troco ---
  ...
  useEffect(() => {
    ...
    setTotal(newTotal);
    if(isPaymentModalOpen && selectedPaymentMethod === 'Dinheiro') {
        const received = parseFloat(String(amountReceived).replace(',', '.'));
        if (!isNaN(received)) { setChangeDue(received - newTotal); } 
        else { setChangeDue(0 - newTotal); }
    }
  }, [cart, isPaymentModalOpen, selectedPaymentMethod, amountReceived]); // total foi removido da dependência para evitar loops
  ...
  const handleAmountReceivedChange = (e) => {
    ...
    if (value !== '') {
        ...
        if (!isNaN(received)) { setChangeDue(received - total); } 
        else { setChangeDue(0 - total); }
    } else { setChangeDue(0 - total); }
  };
  ...
  const handleConfirmPayment = async () => {
    ...
    if (selectedPaymentMethod === 'Dinheiro') {
        const received = parseFloat(String(amountReceived).replace(',', '.'));
        if (isNaN(received) || received < total) { setErrorMessage('O valor recebido é insuficiente ou inválido.'); return; }
    }
    ...
    const saleData = {
      ...
      total: total, metodo_pagamento: finalPaymentMethod,
      nome_cliente: customerName.trim() || 'PDV', custo_pagamento: parseFloat(custoPagamento)
    };
    try {
      ...
      setIsPaymentModalOpen(false); setIsCartOpenMobile(false); setIsSuccessModalOpen(true);
      setCart([]); setCustomerName(''); fetchProducts();
    } catch (error) {
        setErrorMessage(error.message); setIsPaymentModalOpen(false);
    }
  };
  ...
  const handlePrint = async (saleId) => {
    ...
    try {
      ...
      const vendaData = await res.json(); generateAndPrintReceipt(vendaData);
    } catch (error) { console.error(error); setErrorMessage(error.message); }
  };
      ...
      {isPaymentModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm m-4 max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-6 text-center">Total a Pagar: {total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</h2>
            ...
            <div className="border-t pt-4 space-y-3 animate-fade-in">
                <div>
                    ...
                    <div className="mt-1 relative rounded-md shadow-sm">
                        ...
                        <input type="text" inputMode='decimal' name="amountReceived" id="amountReceived"
                ...
                {(amountReceived && !isNaN(parseFloat(String(amountReceived).replace(',','.')))) && (
